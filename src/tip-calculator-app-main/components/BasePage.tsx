...
export const BasePage = () => {
    ...
    const bill = document.getElementById("billInput") as HTMLElement
    const people  = document.getElementById("peopleInput") as HTMLElement
    const custom = document.getElementById("custom") as HTMLElement

    const handleSubmit = (event:number) => {
        setTotal(() => {
            return  (eval(billInput / peopleInput))
         })

    
        setTip(() => {
            return (eval((billInput / peopleInput) * event / 100))
        })

        if(billInput !== '') {
            bill.style.setProperty('border-color', '')
        }

        else{
            bill.style.setProperty('border-color', 'red')
            setTip(0)
            setTotal(0  )
        }
        ...
        if(peopleInput !== '') {
            people.style.setProperty('border-color', '')
        }

        else{
            people.style.setProperty('border-color', 'red')
            setTip(0)
            setTotal(0)
        }
    ...
    const handleCustom = (event) => {
        if(event.key === 'Enter') {
            ...
            setTip(() => {
                return (eval((billInput / peopleInput) * custom.value / 100))
            })
        }
    }
            ...
            className=' p-3 flex flex-col gap-10'>
                <div className='flex flex-col gap-2'>
                    ...
                    <div className='flex items-center'>
                        ...
                        className={`bg-Very-light-grayish-cyan border-Very-light-grayish-cyan
                         text-2xl font-bold text-Very-dark-cyan border-2 focus:outline-Strong-cyan rounded-md w-full h-[3rem]  relative text-right pe-5`} id='billInput' type="number" placeholder='0' />
                ...
                <div className='flex flex-col gap-5'>
                    ...
                    <div className='grid grid-cols-2 md:grid-cols-3 gap-5'>
                        <button value={5} onClick={(e) => handleSubmit(e.target.value)}
                        ...
                        <button value={10} onClick={(e) => handleSubmit(e.target.value)}
                        className='w-full lg:w-[7rem] text-2xl hover:bg-Strong-cyan hover:text-Very-dark-cyan duration-500 font-bold rounded-lg bg-Very-dark-cyan text-white py-3'>
                            10%
                        </button>

                        <button value={15} onClick={(e) => handleSubmit(e.target.value)}
                        className='w-full lg:w-[7rem] text-2xl hover:bg-Strong-cyan hover:text-Very-dark-cyan duration-500 font-bold rounded-lg bg-Very-dark-cyan text-white py-3'>
                            15%
                        </button>

                        <button value={25} onClick={(e) => handleSubmit(e.target.value)}
                        className='w-full lg:w-[7rem] text-2xl hover:bg-Strong-cyan hover:text-Very-dark-cyan duration-500 font-bold rounded-lg bg-Very-dark-cyan text-white py-3'>
                            25%
                        </button>

                        <button value={50} onClick={(e) => handleSubmit(e.target.value)}
                        className='w-full lg:w-[7rem] text-2xl hover:bg-Strong-cyan hover:text-Very-dark-cyan duration-500 font-bold rounded-lg bg-Very-dark-cyan text-white py-3'>
                            50%
                        </button>
